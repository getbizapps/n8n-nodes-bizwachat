import { BizwachatNotifierApi } from '../../credentials/BizwachatNotifierApi.credentials';
import { IAuthenticateGeneric, ICredentialTestRequest } from 'n8n-workflow';

describe('BizwachatNotifierApi Credentials', () => {
	let credentials: BizwachatNotifierApi;

	beforeAll(() => {
		credentials = new BizwachatNotifierApi();
	});

	test('should have the correct name', () => {
		expect(credentials.name).toBe('bizwachatNotifierApi');
	});

	test('should have the correct display name', () => {
		expect(credentials.displayName).toBe('Bizwachat Notifier Trigger API');
	});
	...
	test('should have required properties', () => {
		...
		expect(apiKeyProperty.description).toBe('API Key for Bizwachat Notifier');
	...
	test('should have correct test configuration', () => {
		...
		expect(test.request.url).toBe('/n8n/notifier/webhook');
		...
		expect(test.request.baseURL).toContain('/api');
	});
}); 