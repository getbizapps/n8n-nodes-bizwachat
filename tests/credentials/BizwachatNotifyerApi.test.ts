import { BizwachatNotifyerApi } from '../../credentials/BizwachatNotifyerApi.credentials';
import { IAuthenticateGeneric, ICredentialTestRequest } from 'n8n-workflow';

describe('BizwachatNotifyerApi Credentials', () => {
	let credentials: BizwachatNotifyerApi;

	beforeAll(() => {
		credentials = new BizwachatNotifyerApi();
	});

	test('should have the correct name', () => {
		expect(credentials.name).toBe('bizwachatNotifyerApi');
	});

	test('should have the correct display name', () => {
		expect(credentials.displayName).toBe('Bizwachat Notifyer System Trigger API');
	});
	...
	test('should have required properties', () => {
		...
		expect(apiKeyProperty.description).toBe('API Key for Bizwachat');
